...
export const getOrCreateSection = (
  ...
  const capitalizedName = capitalize(sectionName);
  let section = subject.sections.find(s => s.name === capitalizedName);
  if (!section) {
    section = {
...
export const parseRegistrationDates = (
  ...
  for (let i = 0; i < cols.length; i++) {
    ...
    // Must have exactly 2 <br> tags (creating 3 parts) for the registration dates column
    if (brMatches && brMatches.length >= 2) {
      const parts = cellHtml.split(/<br\s*\/?>/i);

      // Verify this is actually the dates column by checking if at least one part has a date
      const hasDatePattern = parts.some(part => part.match(/\d{2}\.\d{2}\.\d{4}/));
      if (!hasDatePattern) {
        continue; // Not the right column, keep searching
      }
