...
export function GradeBarChart({ grades, order, colors, max }: GradeBarChartProps) {
    ...
    return (
        <div className="flex justify-center items-end gap-3 px-1 mb-8 relative" style={{ height: `${H}px` }}>
            {order.map((g: string) => {
                const v = grades[g] || 0, h = (v / max) * MAX_B;
                return (
                    <div key={g} className="flex flex-col items-center gap-1 group" style={{ flex: '0 0 calc((100% - 6 * 0.75rem) / 7)' }}>
