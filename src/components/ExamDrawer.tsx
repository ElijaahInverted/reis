...
export function ExamDrawer({ isOpen, onClose }: ExamDrawerProps) {
    ...
    // Accordion Control & Highlighting
    const [expandedSubjectId, setExpandedSubjectId] = useState<string>("");
    const [highlightedSectionId, setHighlightedSectionId] = useState<string | null>(null);
    const [activeDateSelectSectionId, setActiveDateSelectSectionId] = useState<string | null>(null);
    ...
    const toggleEdit = (sectionId: string) => {
        setEditingSections(prev => {
            // If currently editing this section, turn it off (clear all)
            if (prev[sectionId]) {
                return {};
            }
            // Otherwise, turn this one on and clear others
    ...
    const handleCalendarDateClick = (date: Date) => {
        ...
        // 1. Priority: Check for Registered Exams on this date
        for (const subject of exams) {
            for (const section of subject.sections) {
                if (section.status === 'registered' && section.registeredTerm?.date && isSameDate(section.registeredTerm.date)) {
                    // If already expanded (second click), switch to edit mode
                    // If already expanded (second click), switch to edit mode
                    if (expandedSubjectId === subject.id) {
                        // Exclusive edit mode: clear others
                        setEditingSections({ [section.id]: true });
                    }
        ...
        // 2. Secondary: Check for Available Terms on this date
        // ONLY if we are currently selecting dates for a specific section (green dots are visible)
        if (activeDateSelectSectionId) {
            for (const subject of exams) {
                const section = subject.sections.find(s => s.id === activeDateSelectSectionId);
                if (section) {
                    const termOnDate = section.terms.find(t => isSameDate(t.date) && !t.full);

                    if (termOnDate) {
                        setSelections(prev => ({
                            ...prev,
                            [section.id]: { date: termOnDate.date, time: undefined }
                        }));
    ...
    const highlightedDates = useMemo(() => {
        if (!activeDateSelectSectionId) return [];

        // Find the section
        let foundSection: ExamSection | undefined;
        for (const subject of exams) {
            foundSection = subject.sections.find(s => s.id === activeDateSelectSectionId);
        ...
        const dates: Date[] = [];
        foundSection.terms.forEach(term => {
            // Filter out full terms unless it's the currently registered one
            if (term.full && !(foundSection.status === 'registered' && term.date === foundSection.registeredTerm?.date)) {
                return;
            }
        ...
        return dates;
    }, [activeDateSelectSectionId, exams]);
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end items-stretch p-4 isolate">
            ...
            {/* Wrapper for Calendar + Drawer */}
            <div className="w-full flex justify-end items-start h-full pt-10 pb-10 relative z-10 pointer-events-none">

                {/* Floating Calendar */}
                <div className="hidden xl:flex flex-1 justify-center items-start pt-32 animate-in fade-in zoom-in-95 duration-500 pointer-events-auto">
                    <ExamCalendar
                        exams={exams}
                ...
                <div className="w-[800px] bg-white shadow-2xl rounded-2xl flex flex-col overflow-hidden border border-gray-100 font-sans h-full animate-in slide-in-from-right duration-300 pointer-events-auto">
                                            ...
                                            <Accordion.Content className="overflow-hidden data-[state=open]:animate-accordion-down data-[state=closed]:animate-accordion-up">
                                                <div className="px-6 py-4 bg-slate-50/50 space-y-1">
                                                    {subject.sections.map((section) => {
                                                        ...
                                                        // Only show dates that have at least one non-full term OR are the currently registered date
                                                        const uniqueDates = Array.from(new Set(
                                                            section.terms.map(t => t.date)
                                                        ));
                                                        ...
                                                        return (
                                                            <div
                                                                key={section.id}
                                                                className={`
                                                                    group flex items-center gap-4 p-3 rounded-lg transition-all border
                                                                    ${isHighlighted
                                                                        ...
                                                                        <div className="flex items-center gap-2 w-full">
                                                                            ...
                                                                            <div className="w-[160px] shrink-0">
                                                                                <Select
                                                                                    ...
                                                                                    onOpenChange={(isOpen) => setActiveDateSelectSectionId(isOpen ? section.id : null)}
                                                                                >
                                                                                    ...
                                                                                    <SelectContent>
                                                                                        {uniqueDates.map(date => {
                                                                                            // Check if all terms for this date are full (and not registered)
                                                                                            const isFull = section.terms
                                                                                                .filter(t => t.date === date)
                                                                                                .every(t => t.full && !(section.status === 'registered' && t.date === section.registeredTerm?.date));
