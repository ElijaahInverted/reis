        ...
        <div className="border-t border-base-200 pt-1">
            <div className="flex items-center justify-between gap-3 px-1 py-2 rounded-lg hover:bg-base-200 cursor-pointer" onClick={() => !loading && onToggle()}>
                <div className="flex items-center gap-2 flex-1">
                    ...
                    <span className="text-xs opacity-70">{t('sync.schedule') as string}</span>
                </div>
                <div className="relative flex items-center gap-2" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
                    ...
                    <AnimatePresence>
                        {show && (
                            <motion.div 
                                ...
                                exit={{ opacity: 0 }} 
                                className="absolute bottom-full right-0 mb-2 w-64 bg-base-200 rounded-lg shadow-lg border p-3 z-[100]"
                            >
                                <p className="text-xs leading-relaxed">{t('sync.tooltip') as string}</p>
                            </motion.div>
                        )}
                    </AnimatePresence>
                    <input 
