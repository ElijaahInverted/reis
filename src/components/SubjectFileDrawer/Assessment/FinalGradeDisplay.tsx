        ...
        <div className="flex-shrink-0 border-t border-base-300 bg-base-200/50 p-4">
            <div className="flex items-center justify-between">
                <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-2xl font-bold text-base-content">{language === 'cz' ? finalGrade.percentage.toFixed(1).replace('.', ',') : finalGrade.percentage.toFixed(1)}%</span>
                    <span className="text-sm text-base-content/40">({language === 'cz' ? finalGrade.totalScore.toFixed(1).replace('.', ',') : finalGrade.totalScore.toFixed(1)} / {finalGrade.totalMax})</span>
                </div>
            </div>
